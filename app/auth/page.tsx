    ...
    <div className="max-w-md mx-auto pt-4">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-500 text-sm mt-1">
          {tab === "login" ? "Sign in to access your funding dashboard" : "Free to start — 5 searches included"}
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4">
        ...
        <div>
          ...
          <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder={tab === "signup" ? "At least 8 characters" : "••••••••"} required className={inp} />
        </div>
        ...
        {tab === "login" && (
          <p className="text-center text-xs text-gray-400">
            No account?{" "}
            <button type="button" onClick={() => setTab("signup")} className="text-[#1a5c3a] font-medium hover:underline">
              Sign up free
            </button>
          </p>
        )}
